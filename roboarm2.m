...
switch modeString
    ...
    case 'demoOrientationControl'
        ...
        trajx = 6;
    case 'demoVIE'
        % Verified only by VIE not by actual Robot yet
        % Has end-effector pointed in downward and in direction from
        % vector [0 0 400] to goalPos.
        % Closes the gripper
        useVelocityControl = 0; % whether to use velocity-only control
...
%If using marker demo, close gripper on marker
if useOrientationDown
    pause(1);
    q=[0 0 0 0 0 0 0 .2].';
    hCyton.setJointParameters(q);
    pause(1);
...
% trajx = 4  is an arc in the x-y plane, use with marker
% trajx = 5  is an arc in the x-y plane, and sinusoid in x-z plane
% trajx = 6  is up and down
% trajx = 7  is a single point
    ...
    if posDiff < goalMargin && ~holdPosition % within some margin
        ...
        if useVelocityControl
            ...
            elseif useOrientationControl && useFlock
             ...
             elseif useOrientationDown
                % compute the goal pointing orientation as the vector from
                % [0 0 400] to goalPos
                pivotPos=[0 0 400];
                goalOrient = (goalPos-pivotPos.').';
                goalOrient = (goalOrient(:).'/norm(goalOrient(:))).'; % make unit vector
                 updateCommand = 1; % update
                
                q(8) = 0.2;hCyton.setJointParameters(q); % close the gripper
            end
        ...
        elseif useAttraction && useFlock %--- Attraction/Chase sequence
            ...
            repelVec = endeffPos.' - repelPos; % vector of attraction
            repelDist = norm(repelVec); % 3D distance, mm
            ...
            if repelDist > attractionGoal
                ...
                if useAttractionCommand
                    ...
                    % degrees, then attract otherwise, don't
